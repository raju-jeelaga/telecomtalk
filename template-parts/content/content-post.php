...
<div class="highlights-content-wrap">
	<div class="h-c">
		...
		<div class="sp-highlights">
			<?php if(isset($tct_fields['enable_hightlights']) && $tct_fields['enable_hightlights'] ){ ?>
			...
			<div class="subs-sec">
				<h3>Follow Us</h3>
		    	<ul>
		    		<?php if(!empty($fb_count)){ ?>
		    		<li>
		    			<a href="https://www.facebook.com/TelecomTalk/" rel="noreferrer">
		    				<span class="f-b"><span class="icon-facebook"></span>Facebook</span>
		    			</a>
		    			<span><?php echo $fb_count;?></span> 
		    		</li>
		    		<?php } ?>
		    		<?php if( !empty($twitter_count)){ ?>
		    		<li>
		    			<a href="https://twitter.com/telecomtalk" rel="noreferrer">
		    				<span class="t-w"><span class="icon-twitter"></span>Twitter</span>
		    			</a>
		    			<span><?php echo $twitter_count;?></span>
		    		</li>
		    		<?php } ?>
		    		<?php if( !empty($telegram_count)){ ?>
		    		<li>
		    			<a href="https://T.me/@telecomtalkinfo" rel="noreferrer">
		    				<span class="tele-gram"><span class="icon-telegram"></span>Telegram</span>
		    			</a>
		    			<span><?php echo $telegram_count;?></span>
		    		</li>
		    		<?php } ?>
		    		<?php if( !empty($subsc_count)){ ?>
		    		<li>
		    			<a href="https://www.youtube.com/telecomtalk" rel="noreferrer">
			    			<span class="you-tb"><span class="icon-youtube"></span>Subscribers</span>
		    			</a>
		    			<span><?php echo $subsc_count;?></span>
		    		</li>
		    		<?php } ?>
		    	</ul>
		    </div>
			...
			<?php 
			 $query = new WP_Query( 
				    	array( 
				    		'category_name' => 'web-stories',
				    		'posts_per_page' => 4, 
				    		'order' => 'DESC',
				    		'post_status' => 'publish') 
				    );
			 $ws_posts = $query->get_posts();
			if(count($ws_posts)){?>  
			<div class="tc_web_stories">
				<h3>Web Stories</h3>
				<ul> 
				<?php
			    	foreach( $ws_posts as $wsvalue ) {
			    	$content = $wsvalue->post_content;
		    		$categories = get_the_category($wsvalue->ID);
		    		if ( has_shortcode( $content, 'web_stories_embed' ) ) {
		    			preg_match('/\[web_stories_embed\surl="(.*?)"(.*?)\]/', $content,$matches);
		    			$web_story_url = $matches[1];
		    			$featured_img_url = get_the_post_thumbnail_url($wsvalue->ID,'full');
		    			echo '<li><a class="wb-img" href="'.$web_story_url.'" target="_blank"><img src="'.$featured_img_url.'" alt="" width="50" height="66"></a>';
		        		echo '<a class="wb-tlt" href="'.$web_story_url.'" target="_blank">'.get_the_title($wsvalue->ID).'</a></li>'; 
			    		}
				    }
				?>
				</ul>
		...
		?>
			<li>
				<div class="video-<?php echo $i;?>">
					...
					<?php } ?>
					<h4><a rel="noreferrer" href="<?php echo get_option($video_link);?>" target="_blank"><?php echo get_option($video_title);?></a></h4>
				</div>
			</li>
		<?php
			}
